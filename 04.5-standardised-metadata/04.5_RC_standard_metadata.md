...
development.[ ]{.Apple-converted-space}
...
extraction).[ ]{.Apple-converted-space}
...
consistent across multiple coders.[  ]{.Apple-converted-space}For
...
]{.Apple-converted-space}If the papers are all within from the finance
...
to create additional labelled datasets. [ ]{.Apple-converted-space}
...
annotations is to *evaluate* the model.[  ]{.Apple-converted-space}How
...
]{.Apple-converted-space}But what if the model returned everything as a
...
]{.Apple-converted-space}But again, if the model returned only one right
answer and no wrong ones, the precision would be perfect. So, it is
important to measure both precision and recall.[ 
]{.Apple-converted-space}In summary, the model in this example, got 80%
